...
class AdaBoost:
    ...
    def stump_error(self, y, y_pred, sample_weights):
        ...
        return (sum(sample_weights * (np.not_equal(y, y_pred))))/(sum(sample_weights)+1e-9)
    ...
    def compute_alpha(self, error):
        ...
        """
        eps = 1e-9
        return np.log((1 - error + eps) / (error + eps))/2.0
    ...
    def update_weights(self, y, y_pred, sample_weights, alpha):
        ...
        """
        temp = sample_weights
        temp = sample_weights * np.exp(np.array([1 if y[x]!=y_pred[x] else -1 for x in range(len(y))])*alpha)
        return temp/(sum(temp)+1e-9)
    ...
    def predict(self, X):
        ...
        for i in X:
            ...
            for j in range(self.n_stumps):
                temp = i.reshape(1, -1)
                yhat = self.stumps[j].predict(temp)[0]
                if(yhat in votes):
                    votes[yhat] += self.alphas[j]
                else:
                    votes[yhat] = self.alphas[j]

            res.append(max(votes, key=votes.get))
        return res
