...
class Tensor:
    ...
    def grad_add(self, gradients=None):
        ...
        if gradients is None:
            left, right = np.ones((self.shape[0], self.shape[1])), np.ones((self.shape[0], self.shape[1]))
        else:
    ...
    def grad_matmul(self, gradients=None):
        ...
        left, right = None, None
        if gradients is None:
            gradients = np.ones((self.shape[0], self.shape[1]))

        left, right = np.dot(gradients, child[2].arr.T), np.dot(child[1].arr.T, gradients)
