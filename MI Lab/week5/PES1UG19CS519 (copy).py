...
class Tensor:
    ...
    def grad_add(self, gradients=None):
        ...
        try:
            if(gradients is None):
                if(child[1] != child[2]):
                    gradients = np.ones((child[1].shape[0],child[2].shape[1]))
                else:
                    gradients = 2*np.ones((child[1].shape[0],child[2].shape[1]))

            left, right = gradients, gradients
    ...
    def grad_matmul(self, gradients=None):
        ...
        left = np.dot(gradients,((child[2]).arr).T)
        right = np.dot(((child[1]).arr).T,gradients) 
    ...
    def backward(self, gradients=None):
        ...
        if(child[0] == 'add'):
            left, right = self.grad_add(gradients)

        elif(child[0] == 'matmul'):
            left, right = self.grad_matmul(gradients)

        if(child[1].history[0] != 'leaf'):
            child[1].backward(left)
        else:
            if(child[1].requires_grad):
                child[1].grad = left
            
        # if(child[1] != child[2]):
        if(child[2].history[0] != 'leaf'):
            child[2].backward(right)
        else:
            if(child[2].requires_grad):
                child[2].grad = right
